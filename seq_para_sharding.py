...
@functional_datapipe('seq_sharding')
class SeqShardingDataPipe(IterDataPipe):
    r"""
    ...
    def __iter__(self):
        for i, item in enumerate(self.source_datapipe):
            ...
            local_instance_id = self.instance_id % instances_per_gpu
            sp_group_id = 0
            sp_groups = 1
            sp_instances = sp_groups * instances_per_gpu
            sp_instance_id = sp_group_id * instances_per_gpu + local_instance_id
            if i % self.sp_instances == self.sp_instance_id:
                yield item
